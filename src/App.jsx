...
// ─── SUPABASE HELPERS ─────────────────────────────────────────────────────────
async function sbGet(table, filters = {}) {import { useState, useEffect, useRef } from "react";
import * as XLSX from "xlsx";
  ...
  if (f.error) return (
    <div style={{ background: "#1a0a0a", border: "1px solid #3b1f1f", borderRadius: 10, padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ color: "#f87171", fontSize: 12, flex: 1 }}>❌ {f.archivo}: {f.error}</span>
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button onClick={() => onUpdate(f.id, "_eliminar", true)}
          style={{ background: "rgba(248,113,113,.1)", color: "#f87171", border: "1px solid rgba(248,113,113,.2)", borderRadius: 6, padding: "4px 12px", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>
          🗑 Eliminar
        </button>
      </div>
    </div>
...
function PanelAuditoriaCuentas({ facturas, onClose, onUpdate, pucCuentas }) {
  ...
  facturas.filter(f => f.asiento).forEach(f => {
    (f.asiento||[]).forEach(r => {
      ...
      if (!cuentaMap[r.cuenta]) cuentaMap[r.cuenta] = {
        cuenta: r.cuenta, nombre: r.descripcion || r.cuenta,
        debito: 0, credito: 0, facturas: []
      };
  ...
  // Sugerencias de cuentas para el cambio masivo
  const sugerencias = (pucCuentas||[]).filter(p =>
    busqCuenta.length > 1 &&
    (p[0]?.toString().includes(busqCuenta) || p[1]?.toLowerCase().includes(busqCuenta.toLowerCase()))
  ).slice(0,8);

  // Cambiar cuenta en facturas seleccionadas
  const aplicarCambio = () => {
    if (!nuevaCuenta || seleccionados.size === 0) return;
    const ctaOld = cuentaSel;
    seleccionados.forEach(fid => {
      const fac = cuentaMap[ctaOld]?.facturas.find(f => f.id === fid);
      if (!fac) return;
      const nuevoAsiento = (fac.asiento||[]).map(r =>
        r.cuenta === ctaOld ? { ...r, cuenta: nuevaCuenta } : r
      );
      onUpdate(fid, "asiento", nuevoAsiento);
    });
  ...
  // Editar asiento de una factura individual
  const editarFila = (fac, cta, nuevaCtaFila) => {
    const nuevoAsiento = (fac.asiento||[]).map(r =>
      r.cuenta === cta ? { ...r, cuenta: nuevaCtaFila } : r
    );
    onUpdate(fac.id, "asiento", nuevoAsiento);
  };

  const facturasDelCuenta = cuentaSel ? (cuentaMap[cuentaSel]?.facturas || []) : [];

  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div style={{ background:"#161923",border:"1px solid #232840",borderRadius:16,width:"100%",maxWidth:1000,maxHeight:"92vh",display:"flex",flexDirection:"column" }}>
        ...
        <div style={{ padding:"14px 20px",borderBottom:"1px solid #1e2235",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap" }}>
          ...
          <span style={{ fontSize:11,color:"#64748b" }}>{cuentas.length} cuentas · {facturas.length} facturas</span>
          {cuentaSel && seleccionados.size > 0 && (
            <button onClick={()=>setModalCambio(true)}
              style={{ background:"rgba(251,191,36,.15)",color:"#fbbf24",border:"1px solid rgba(251,191,36,.3)",borderRadius:6,padding:"4px 12px",cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"inherit" }}>
              ✏ Cambiar cuenta en {seleccionados.size} seleccionadas
            </button>
          )}
          {cuentaSel && facturasDelCuenta.length > 0 && (
            <button onClick={()=>{
              if (seleccionados.size === facturasDelCuenta.length) setSeleccionados(new Set());
              else setSeleccionados(new Set(facturasDelCuenta.map(f=>f.id)));
            }} style={{ background:"transparent",border:"1px solid #2d3352",color:"#64748b",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:11,fontFamily:"inherit" }}>
              {seleccionados.size === facturasDelCuenta.length ? "Deseleccionar todas" : "Seleccionar todas"}
            </button>
          )}
        ...
        {/* Body */}
        <div style={{ flex:1,overflow:"hidden",display:"grid",gridTemplateColumns:cuentaSel?"1fr 1fr":"1fr" }}>

          {/* Lista de cuentas */}
          <div style={{ overflowY:"auto",borderRight:cuentaSel?"1px solid #1e2235":"none" }}>
            <table style={{ width:"100%",borderCollapse:"collapse",fontSize:12 }}>
              <thead>
                <tr style={{ background:"#0d101a",position:"sticky",top:0 }}>
                  {["Cuenta","Nombre","Débito","Crédito","Facts"].map(h=>(
                    <th key={h} style={{ padding:"8px 12px",textAlign:["Débito","Crédito","Facts"].includes(h)?"right":"left",fontSize:10,fontWeight:700,color:"#475569",textTransform:"uppercase",letterSpacing:.5,borderBottom:"1px solid #1e2235" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {cuentas.map((ct,i) => (
                  <tr key={ct.cuenta} onClick={()=>{ setCuentaSel(cuentaSel===ct.cuenta?null:ct.cuenta); setSeleccionados(new Set()); }}
                    style={{ borderBottom:"1px solid rgba(30,34,58,.5)",cursor:"pointer",
                      background:cuentaSel===ct.cuenta?"rgba(79,142,247,.12)":i%2===0?"transparent":"rgba(255,255,255,.01)" }}>
                    <td style={{ padding:"8px 12px",fontFamily:"monospace",color:"#e8b84b",fontWeight:600 }}>{ct.cuenta}</td>
                    <td style={{ padding:"8px 12px",color:"#94a3b8",maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{ct.nombre}</td>
                    <td style={{ padding:"8px 12px",textAlign:"right",fontFamily:"monospace",color:"#4ade80",fontSize:11 }}>{ct.debito>0?fmt(ct.debito):"—"}</td>
                    <td style={{ padding:"8px 12px",textAlign:"right",fontFamily:"monospace",color:"#f87171",fontSize:11 }}>{ct.credito>0?fmt(ct.credito):"—"}</td>
                    <td style={{ padding:"8px 12px",textAlign:"right",color:"#64748b",fontSize:11 }}>{ct.facturas.length}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {cuentaSel && (
            <div style={{ overflowY:"auto" }}>
              <div style={{ padding:"10px 14px",borderBottom:"1px solid #1e2235",background:"#0d101a",position:"sticky",top:0 }}>
                <div style={{ fontSize:12,fontWeight:700,color:"#4f8ef7" }}>Cuenta {cuentaSel}</div>
                <div style={{ fontSize:11,color:"#64748b" }}>{cuentaMap[cuentaSel]?.nombre} · {facturasDelCuenta.length} facturas</div>
              </div>
              {facturasDelCuenta.map((f,i) => {
                const linea = (f.asiento||[]).find(r=>r.cuenta===cuentaSel);
                const isSel = seleccionados.has(f.id);
                return (
                  <div key={f.id} style={{ padding:"12px 14px",borderBottom:"1px solid rgba(30,34,58,.5)",background:isSel?"rgba(251,191,36,.05)":"transparent" }}>
                    <div style={{ display:"flex",alignItems:"flex-start",gap:8 }}>
                      ...
                      <div style={{ flex:1,minWidth:0 }}>
                        <div style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}>
                          <span style={{ fontWeight:600,color:"#e2e8f0",fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{f.razonSocial||f.nitProveedor}</span>
                          <span style={{ fontFamily:"monospace",color:"#e8b84b",fontSize:11,flexShrink:0,marginLeft:8 }}>{f.prefijo||"—"}</span>
                        </div>
                        <div style={{ fontSize:11,color:"#64748b",marginBottom:4 }}>NIT {f.nitProveedor} · {f.fecha}</div>
                        {linea && (
                      ...
                      {/* Botón editar esta factura */}
                      <button onClick={()=>setFacturaEdit(facturaEdit?.id===f.id?null:f)}
                        style={{ background:"rgba(79,142,247,.1)",color:"#4f8ef7",border:"1px solid rgba(79,142,247,.3)",borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:11,fontFamily:"inherit",flexShrink:0 }}>
                        ✏
                      </button>
                    </div>
                    {/* Editor inline de asiento */}
                    {facturaEdit?.id===f.id && (
                      <div style={{ marginTop:10,background:"#0d101a",border:"1px solid #1e2235",borderRadius:8,padding:"10px 12px" }}>
                        <div style={{ fontSize:11,fontWeight:600,color:"#4f8ef7",marginBottom:8 }}>Asiento — {f.razonSocial}</div>
                        {(f.asiento||[]).map((r,ri) => (
                          <div key={ri} style={{ display:"flex",gap:8,alignItems:"center",marginBottom:6 }}>
                            <span style={{ fontSize:10,padding:"2px 6px",borderRadius:4,background:r.tipo==="debito"?"rgba(74,222,128,.1)":"rgba(248,113,113,.1)",color:r.tipo==="debito"?"#4ade80":"#f87171",fontWeight:600,flexShrink:0 }}>{r.tipo==="debito"?"DÉB":"CRÉ"}</span>
                            <input
                              defaultValue={r.cuenta}
                              onBlur={e=>{
                                if(e.target.value!==r.cuenta) {
                                  const nuevoAsiento=(f.asiento||[]).map((x,xi)=>xi===ri?{...x,cuenta:e.target.value}:x);
                                  onUpdate(f.id,"asiento",nuevoAsiento);
                                }
                              }}
                              style={{ width:110,background:"#161923",border:"1px solid #2d3352",color:"#e8b84b",borderRadius:4,padding:"3px 6px",fontFamily:"monospace",fontSize:11,outline:"none" }}
                            />
                            <span style={{ fontSize:11,color:"#64748b",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{r.descripcion}</span>
                            <span style={{ fontFamily:"monospace",fontSize:11,color:r.tipo==="debito"?"#4ade80":"#f87171",flexShrink:0 }}>{fmt(r.valor)}</span>
                          </div>
                        ))}
                        <div style={{ fontSize:10,color:"#475569",marginTop:6 }}>Edite el código de cuenta y haga clic afuera para guardar</div>
                      </div>
                    )}
                  </div>
                );
      ...
      {modalCambio && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:4000,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
          <div style={{ background:"#161923",border:"1px solid #232840",borderRadius:12,padding:24,maxWidth:420,width:"100%" }}>
            <div style={{ fontWeight:700,fontSize:14,color:"#fff",marginBottom:4 }}>✏ Cambiar cuenta en {seleccionados.size} facturas</div>
            <div style={{ fontSize:11,color:"#64748b",marginBottom:16 }}>
              Cuenta actual: <span style={{ color:"#e8b84b",fontFamily:"monospace" }}>{cuentaSel}</span> → {cuentaMap[cuentaSel]?.nombre}
            </div>
            <div style={{ marginBottom:8 }}>
              <div style={{ fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:.5,marginBottom:4 }}>Nueva cuenta</div>
              <input value={busqCuenta} onChange={e=>{setBusqCuenta(e.target.value);setNuevaCuenta(e.target.value);}}
                placeholder="Código o nombre de cuenta..."
                style={{ width:"100%",background:"#0d101a",border:"1px solid #2d3352",color:"#e2e8f0",borderRadius:6,padding:"8px 10px",fontFamily:"monospace",fontSize:12,outline:"none" }}
              />
            </div>
            {sugerencias.length > 0 && (
              <div style={{ background:"#0d101a",border:"1px solid #1e2235",borderRadius:6,marginBottom:12,maxHeight:160,overflowY:"auto" }}>
                {sugerencias.map(s=>(
                  <div key={s[0]} onClick={()=>{setNuevaCuenta(String(s[0]));setBusqCuenta(s[0]+" – "+s[1]);}}
                    style={{ padding:"7px 12px",cursor:"pointer",borderBottom:"1px solid #1e2235",display:"flex",gap:10,alignItems:"center" }}>
                    <span style={{ fontFamily:"monospace",color:"#e8b84b",fontSize:12,minWidth:80 }}>{s[0]}</span>
                    <span style={{ fontSize:11,color:"#94a3b8" }}>{s[1]}</span>
                  </div>
                ))}
              </div>
            )}
            <div style={{ fontSize:11,color:"#475569",marginBottom:14 }}>
              Se cambiará la cuenta <span style={{ color:"#e8b84b",fontFamily:"monospace" }}>{cuentaSel}</span> por <span style={{ color:"#4ade80",fontFamily:"monospace" }}>{nuevaCuenta||"—"}</span> en las facturas seleccionadas
            </div>
            <div style={{ display:"flex",gap:8,justifyContent:"flex-end" }}>
              <button onClick={()=>setModalCambio(false)} style={{ background:"transparent",border:"1px solid #2d3352",color:"#94a3b8",borderRadius:6,padding:"7px 14px",cursor:"pointer",fontSize:12,fontFamily:"inherit" }}>Cancelar</button>
              <button onClick={aplicarCambio} disabled={!nuevaCuenta}
                style={{ background:nuevaCuenta?"#fbbf24":"#1e2235",color:nuevaCuenta?"#0f1f3d":"#475569",border:"none",borderRadius:6,padding:"7px 18px",cursor:nuevaCuenta?"pointer":"not-allowed",fontSize:12,fontWeight:700,fontFamily:"inherit" }}>
                ✓ Aplicar cambio
              </button>
            </div>
          </div>
  ...
  return (
    <div style={{ fontFamily: "monospace", background: "#0f1117", color: "#e2e8f0", display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
      ...
      {modalEnviar && <ModalEnviar facturas={facturas} empresaActual={empresaActual} onClose={() => setModalEnviar(false)} onEnviado={(res) => {
          ...
          // Mover aprobadas al historial de subidas
          const enviadas = facturas
            .filter(f => f.aprobado && !f.error && f.asiento)
            ...
            <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #1e2235", marginBottom: 12 }}>
              {[
                ...
                <button key={id} onClick={() => setTabVista(id)}
                  ...
                  {lbl} <span style={{ marginLeft: 4, background: tabVista === id ? "rgba(74,222,128,.15)" : "rgba(255,255,255,.06)",
                    color: tabVista === id ? "#4ade80" : "#475569", borderRadius: 20, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>{cnt}</span>
                </button>
              ))}
              {subidas.length > 0 && (
                <button onClick={() => { if (window.confirm("¿Limpiar historial?")) guardarSubidas([]); }}
                  style={{ marginLeft: "auto", background: "transparent", border: "none", color: "#475569", cursor: "pointer", fontSize: 11, fontFamily: "monospace", padding: "8px 12px" }}>
                  🗑 Limpiar
                </button>
              )}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {subidas.map((f, i) => (
                <div key={f.id + "_" + i} style={{ background: "#0f1a0f", border: "1px solid #166534", borderRadius: 10, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  <div>
                    <span style={{ fontWeight: 700, color: "#4ade80", fontSize: 13 }}>{f.razonSocial || f.nitProveedor}</span>
                    <span style={{ marginLeft: 10, color: "#475569", fontSize: 11, fontFamily: "monospace" }}>{f.nitProveedor}</span>
                    <div style={{ fontSize: 11, color: "#475569", marginTop: 3 }}>
                      Fecha: {f.fecha} · Subida: {f.fechaSubida || "—"} · Total: <span style={{ color: "#4ade80", fontWeight: 600 }}>${(f.total || 0).toLocaleString("es-CO")}</span>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    ...
                    <button onClick={() => {
                      setFacturas(p => [...p, { ...f, aprobado: false, fechaSubida: undefined }]);
                      guardarSubidas(subidas.filter((_, idx) => idx !== i));
                      setTabVista("pendientes");
                    }} style={{ background: "transparent", border: "1px solid #2d3352", color: "#64748b", borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontSize: 11, fontFamily: "monospace" }}>
                      ↩ Restaurar
                    </button>
                  </div>
                </div>
              ...
              {aprobadasAcum.length > 0 && (
                <div>
                  <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:8,paddingBottom:6,borderBottom:"1px solid #166534" }}>
                    ...
                    <span style={{ fontSize:11,fontWeight:700,color:"#4ade80",textTransform:"uppercase",letterSpacing:.5 }}>
