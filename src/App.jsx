...
const TABLA_RETE = [
  ...
  { t:"compras",   p:"juridica", r:2.5, b:27, fi:"2026-05-08", ff:"2026-06-30" },
  { t:"compras",   p:"juridica", r:2.5, b:10, fi:"2026-07-01", ff:"2099-12-31" },
  { t:"compras",   p:"natural",  r:3.5, b:27, fi:"2023-01-01", ff:"2025-05-31" },
  { t:"compras",   p:"natural",  r:3.5, b:10, fi:"2025-06-01", ff:"2026-05-07" },
  { t:"compras",   p:"natural",  r:3.5, b:27, fi:"2026-05-08", ff:"2026-06-30" },
  { t:"compras",   p:"natural",  r:3.5, b:10, fi:"2026-07-01", ff:"2099-12-31" },
  { t:"servicios", p:"juridica", r:4,   b:4,  fi:"2023-01-01", ff:"2025-05-31" },
  { t:"servicios", p:"juridica", r:4,   b:2,  fi:"2025-06-01", ff:"2026-05-07" },
  { t:"servicios", p:"juridica", r:4,   b:4,  fi:"2026-05-08", ff:"2026-06-30" },
  { t:"servicios", p:"juridica", r:4,   b:2,  fi:"2026-07-01", ff:"2099-12-31" },
  { t:"servicios", p:"natural",  r:4,   b:4,  fi:"2023-01-01", ff:"2025-05-31" },
  { t:"servicios", p:"natural",  r:4,   b:2,  fi:"2025-06-01", ff:"2026-05-07" },
  { t:"servicios", p:"natural",  r:4,   b:4,  fi:"2026-05-08", ff:"2026-06-30" },
  { t:"servicios", p:"natural",  r:4,   b:2,  fi:"2026-07-01", ff:"2099-12-31" },
  { t:"servicios", p:"no_decl",  r:6,   b:4,  fi:"2023-01-01", ff:"2025-05-31" },
  { t:"servicios", p:"no_decl",  r:6,   b:2,  fi:"2025-06-01", ff:"2026-05-07" },
  { t:"servicios", p:"no_decl",  r:6,   b:4,  fi:"2026-05-08", ff:"2026-06-30" },
  { t:"servicios", p:"no_decl",  r:6,   b:2,  fi:"2026-07-01", ff:"2099-12-31" },
  ...
  { t:"transporte",p:"juridica", r:1,   b:4,  fi:"2026-05-08", ff:"2026-06-30" },
  { t:"transporte",p:"juridica", r:1,   b:2,  fi:"2026-07-01", ff:"2099-12-31" },
  { t:"transporte",p:"natural",  r:1,   b:4,  fi:"2023-01-01", ff:"2025-05-31" },
  { t:"transporte",p:"natural",  r:1,   b:2,  fi:"2025-06-01", ff:"2026-05-07" },
  { t:"transporte",p:"natural",  r:1,   b:4,  fi:"2026-05-08", ff:"2026-06-30" },
  { t:"transporte",p:"natural",  r:1,   b:2,  fi:"2026-07-01", ff:"2099-12-31" },
  ...
  { t:"arrend_inmueble", p:"ambas", r:3.5, b:27, fi:"2026-05-08", ff:"2026-06-30" },
  { t:"arrend_inmueble", p:"ambas", r:3.5, b:10, fi:"2026-07-01", ff:"2099-12-31" },
  ...
  { t:"vigilancia",p:"juridica",r:2,   b:4,  fi:"2026-05-08", ff:"2026-06-30" },
  { t:"vigilancia",p:"juridica",r:2,   b:2,  fi:"2026-07-01", ff:"2099-12-31" },
...
function calcularRetencion(tipo, persona, fecha, subtotal, pucCuentas, esAutorretenedor) {
  ...
  const fila = TABLA_RETE.find(r => r.t === tipo && (r.p === "ambas" || r.p === pNorm) && fecha >= r.fi && fecha <= r.ff);
  ...
  if (pucCuentas.length > 0) {
    ...
    const buscar = (terminos) => cands23.find(c => { const nom = (c.nombre || "").toLowerCase(); return terminos.every(t => nom.includes(t.toLowerCase())); });
    if (tipo === "compras" && pNorm === "juridica") cuenta = buscar(["2.5","juridica"]) || buscar(["2.5","compra"]) || buscar(["compra","juridica"]);
    else if (tipo === "compras" && pNorm === "natural") cuenta = buscar(["2.5","natural"]) || buscar(["3.5","natural"]) || buscar(["compra","natural"]);
    else if (tipo === "compras" && pNorm === "no_decl") cuenta = buscar(["3.5","natural"]) || buscar(["3.5","compra"]) || buscar(["compra","natural"]);
    else if (tipo === "honorarios" && pNorm === "juridica") cuenta = buscar(["11","honor","juridica"]) || buscar(["11","honor"]);
    else if (tipo === "honorarios") cuenta = buscar(["10","honor","natural"]) || buscar(["10","honor"]);
    else if (tipo === "servicios" && pNorm === "juridica") cuenta = buscar(["4","serv","juridica"]) || buscar(["4","serv"]);
    else if (tipo === "servicios") cuenta = buscar(["4","serv","natural"]) || buscar(["6","serv"]) || buscar(["serv","natural"]);
    else if (tipo === "transporte" && pNorm === "juridica") cuenta = buscar(["1","transport","juridica"]) || buscar(["1","carga","juridica"]) || buscar(["transport"]);
    else if (tipo === "transporte") cuenta = buscar(["1","transport","natural"]) || buscar(["1","carga"]) || buscar(["transport"]);
    else if (tipo === "arrend_inmueble") cuenta = buscar(["3.5","arrend"]) || buscar(["arrend","inmueble"]) || buscar(["arrend"]);
    else if (tipo === "arrend_mueble") cuenta = buscar(["4","arrend"]) || buscar(["mueble","arrend"]);
    else if (tipo === "obra_civil") cuenta = buscar(["2","obra"]) || buscar(["obra"]);
    else if (tipo === "vigilancia") cuenta = buscar(["2","vigil"]) || buscar(["aseo","vigil"]) || buscar(["vigil"]);
    else if (tipo === "combustibles" && pNorm === "juridica") cuenta = buscar(["0.1","juridica"]) || buscar(["0.1","combust"]) || buscar(["combustible","juridica"]);
    else if (tipo === "combustibles") cuenta = buscar(["0.1","natural"]) || buscar(["0.1","combust"]) || buscar(["combustible","natural"]);
    else if (tipo === "comisiones" && pNorm === "juridica") cuenta = buscar(["11","comis","juridica"]) || buscar(["11","comis"]);
    else if (tipo === "comisiones") cuenta = buscar(["10","comis","natural"]) || buscar(["10","comis"]);
    if (!cuenta) cuenta = buscar([tarifaStr2,"juridica"]) || buscar([tarifaStr2,"natural"]) || buscar([tarifaStr2]);
  }
  return { pct: fila.r, valor, aplica: true, cuenta: cuenta ? { codigo: cuenta.codigo, nombre: cuenta.nombre } : null, nota: `${fila.r}% sobre $${subtotal.toLocaleString("es-CO")} = $${valor.toLocaleString("es-CO")}${basePesos > 0 ? ` (base ${fila.b} UVT)` : " (primer peso)"}` };
}
...
async function upsertTerceroSB(tercero) {
  ...
  try {
    ...
    if (existe && existe.length > 0) {
      // Ya existe — solo actualizar campos vacíos via PATCH
      const actual = existe[0];
      const update = {};
      Object.keys(tercero).forEach(k => {
        if (k === "id" || k === "numero" || k === "empresa_id") return;
        const val = tercero[k];
      ...
      if (Object.keys(update).length > 0) {
        await fetch(`${SB_URL}/rest/v1/terceros?id=eq.${actual.id}`, {
          method: "PATCH",
      ...
      await fetch(`${SB_URL}/rest/v1/terceros`, {
        method: "POST",
        headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}`, "Content-Type": "application/json", Prefer: "return=minimal" },
...
function parseXML(xmlText) {
  try {
    ...
    const additionalAccountID = gn(sup, "AdditionalAccountID") || doc.getElementsByTagNameNS("*", "AdditionalAccountID")[0]?.textContent?.trim() || "1";
...
async function callClaude(body, intento = 0) {
  ...
  if (data.error) {
    if (data.error?.type === "overloaded_error" && intento < 5) { const espera = (intento + 1) * 20000; await sleep(espera); return callClaude(body, intento + 1); }
...
async function analizarConIA(datos, tratamiento, tratIva, pucCuentas) {
  ...
  const pucTexto = pucCuentas.map(c => `${c.codigo}\t${c.nombre}`).join("\n");
  const pucGastos = pucCuentas.filter(c => { const cod = c.codigo || ""; if (tratamiento === "inventario") return cod.startsWith("1"); return cod.startsWith("6") || cod.startsWith("5"); }).map(c => `${c.codigo}\t${c.nombre}`).join("\n");
...
function construirAsiento(datos, ia, rete, pucCuentas, esAutorretenedor, tratIva) {
  ...
  (ia.lineas_contables || []).forEach((l, i) => {
    const val = Math.round(Math.abs(l.valor || 0));
    if (!val) return;
  ...
  const ctaProv = pucCuentas.find(c => /proveedor/i.test(c.nombre) && c.codigo.startsWith("22") && c.codigo.length === 8) || pucCuentas.find(c => /proveedor/i.test(c.nombre) && c.codigo.startsWith("22")) || { codigo: "22050101", nombre: "Proveedores nacionales" };
...
function useSupabase() {
  ...
  useEffect(() => {
    (async () => {
      const [emps, autos] = await Promise.all([sbGet("empresas"), sbGet("dian_autorretenedores")]);
      setEmpresas(emps || []);
      const autoMap = {};
      ...
      if (emp) { setEmpresaActualSt(emp); const puc = await sbGet("puc_auxiliares", { empresa_id: emp.id }); setPucCuentas(puc || []); }
      setCargando(false);
    })();
  }, []);
  const setEmpresaActual = async (emp) => { setEmpresaActualSt(emp); localStorage.setItem("contaia_empresa_nit", emp.nit); setCargando(true); const puc = await sbGet("puc_auxiliares", { empresa_id: emp.id }); setPucCuentas(puc || []); setCargando(false); };
...
function FacturaCard({ f, idx, docNum, onUpdate, onAprender }) {
  ...
  if (f.procesando) return (<div style={{ background: "#1a2035", border: "1px solid #1e3a5f", borderRadius: 10, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}><div style={{ fontSize: 22, animation: "spin 1s linear infinite", display: "inline-block" }}>⚙️</div><div style={{ flex: 1 }}><div style={{ fontSize: 12, color: "#60a5fa", fontWeight: 600 }}>Procesando con IA...</div><div style={{ fontSize: 11, color: "#475569", marginTop: 2, fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.archivo}</div></div><span style={{ fontSize: 10, color: "#475569", background: "#131c2e", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "2px 8px" }}>En cola</span></div>);
  if (f.error) return (<div style={{ background: "#1a0a0a", border: "1px solid #3b1f1f", borderRadius: 10, padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ color: "#64748b", fontSize: 11 }}>#{String(idx + 1).padStart(2, "00")}</span><span style={{ color: "#f87171", fontSize: 12, flex: 1 }}>❌ {f.archivo}: {f.error}</span></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{f.error.toString().includes("DUPLICADO") && (<button onClick={() => { onUpdate(f.id, "error", null); onUpdate(f.id, "aprobado", false); }} style={{ background: "rgba(251,191,36,.15)", color: "#fbbf24", border: "1px solid rgba(251,191,36,.3)", borderRadius: 6, padding: "4px 12px", cursor: "pointer", fontSize: 11, fontFamily: "inherit", fontWeight: 600 }}>⚡ Ignorar duplicado</button>)}{f.xmlOriginal && (<button onClick={() => { onUpdate(f.id, "error", null); onUpdate(f.id, "_reprocesar_xml", true); }} style={{ background: "rgba(96,165,250,.15)", color: "#60a5fa", border: "1px solid rgba(96,165,250,.3)", borderRadius: 6, padding: "4px 12px", cursor: "pointer", fontSize: 11, fontFamily: "inherit", fontWeight: 600 }}>🔄 Reintentar</button>)}{f.trackId && !f.xmlOriginal && (<button onClick={() => onUpdate(f.id, "_redescargar_dian", true)} style={{ background: "rgba(0,229,160,.15)", color: "#00e5a0", border: "1px solid rgba(0,229,160,.3)", borderRadius: 6, padding: "4px 12px", cursor: "pointer", fontSize: 11, fontFamily: "inherit", fontWeight: 600 }}>📥 Re-descargar DIAN</button>)}<button onClick={() => onUpdate(f.id, "_eliminar", true)} style={{ background: "rgba(248,113,113,.1)", color: "#f87171", border: "1px solid rgba(248,113,113,.2)", borderRadius: 6, padding: "4px 12px", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>🗑 Eliminar</button></div></div>);
  return (
    <div id={`factura-${f.id}`} style={{ background: "#1a2035", border: `1px solid ${f.aprobado ? "#166534" : cuadra ? "#232840" : "#7c3700"}`, borderRadius: 12, overflow: "hidden" }}>
      ...
      <div style={{ padding: "11px 16px", display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[{ l: "Subtotal", v: f.subtotal, c: "#cbd5e1" }, { l: "IVA", v: f.totalIva, c: "#60a5fa" }, { l: "Total", v: f.total, c: "#4ade80", big: true }, { l: "Neto", v: neto, c: "#fbbf24", big: true }].map(({ l, v, c, big }) => (<div key={l} style={{ background: "#131c2e", borderRadius: 7, padding: "6px 10px", textAlign: "center", minWidth: 72 }}><div style={{ fontSize: 9, color: "#475569", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 2 }}>{l}</div><div style={{ fontSize: big ? 13 : 11, fontWeight: big ? 700 : 500, color: c, whiteSpace: "nowrap" }}>{fmt(v)}</div></div>))}
        </div>
      </div>
      {expandido && f.ia && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", padding: "12px 16px", background: "#131c2e" }}>
          ...
          <div style={{ background: "#131c2e", borderRadius: 7, overflow: "hidden", border: `1px solid ${cuadra ? "rgba(255,255,255,0.08)" : "#7c3700"}` }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              <thead><tr style={{ background: "#131c2e" }}>{["Tipo", "PlnCod", "Descripción", "Débito", "Crédito", ""].map(h => <th key={h} style={{ textAlign: "left", padding: "6px 9px", color: "#475569", fontSize: 10, fontWeight: 600, textTransform: "uppercase", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>{h}</th>)}</tr></thead>
              <tbody>
                {filas.map(r => (<tr key={r.id} style={{ borderBottom: "1px solid #1a1d27", background: r.advertencia ? "#1a120022" : r.id === "prov" ? "#0f1f3d" : "transparent" }}><td style={{ padding: "6px 9px" }}>{!f.aprobado && r.editable && r.id !== "prov" ? <select value={r.tipo} onChange={e => updFila(r.id, "tipo", e.target.value)} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid #2d3352", color: r.tipo === "debito" ? "#4ade80" : "#f87171", borderRadius: 4, padding: "2px 6px", fontSize: 10, cursor: "pointer" }}><option value="debito">DÉB</option><option value="credito">CRÉ</option></select> : <span style={{ fontSize: 10, fontWeight: 700, color: r.tipo === "debito" ? "#4ade80" : "#f87171", background: r.tipo === "debito" ? "#0a2010" : "#200a0a", padding: "2px 7px", borderRadius: 4 }}>{r.tipo === "debito" ? "DÉB" : "CRÉ"}</span>}</td><td style={{ padding: "6px 9px" }}>{!f.aprobado && r.editable ? <CeldaEditable valor={r.cuenta} onChange={v => updFila(r.id, "cuenta", v)} style={{ fontFamily: "monospace", color: r.advertencia ? "#fb923c" : "#60a5fa", fontWeight: 600 }} /> : <span style={{ fontFamily: "monospace", color: r.advertencia ? "#fb923c" : "#60a5fa", fontWeight: 600 }}>{r.cuenta}</span>}{r.advertencia && <span style={{ marginLeft: 4, fontSize: 9, color: "#fbbf24" }}>⚠</span>}</td><td style={{ padding: "6px 9px", maxWidth: 180 }}>{!f.aprobado && r.editable ? <CeldaEditable valor={r.descripcion} onChange={v => updFila(r.id, "descripcion", v)} style={{ color: "#cbd5e1" }} /> : <span style={{ color: "#94a3b8" }}>{r.descripcion}</span>}</td><td style={{ padding: "6px 9px", textAlign: "right" }}>{r.tipo === "debito" ? (!f.aprobado && r.editable ? <CeldaEditable valor={r.valor} onChange={v => updFila(r.id, "valor", v)} tipo="number" style={{ color: "#4ade80", fontWeight: 600, textAlign: "right" }} /> : <span style={{ color: "#4ade80", fontWeight: 600 }}>{fmt(r.valor)}</span>) : <span style={{ color: "#2d3352" }}>—</span>}</td><td style={{ padding: "6px 9px", textAlign: "right" }}>{r.tipo === "credito" ? (!f.aprobado && r.editable ? <CeldaEditable valor={r.valor} onChange={v => updFila(r.id, "valor", v)} tipo="number" style={{ color: r.id === "prov" ? "#fbbf24" : "#f87171", fontWeight: 600, textAlign: "right" }} /> : <span style={{ color: r.id === "prov" ? "#fbbf24" : "#f87171", fontWeight: r.id === "prov" ? 700 : 600 }}>{fmt(r.valor)}</span>) : <span style={{ color: "#2d3352" }}>—</span>}</td><td style={{ padding: "6px 9px", textAlign: "center" }}>{!f.aprobado && r.eliminable ? <button onClick={() => elimFila(r.id)} style={{ background: "transparent", border: "1px solid #3b1f1f", color: "#f87171", borderRadius: 4, padding: "2px 7px", cursor: "pointer", fontSize: 11 }}>🗑</button> : r.id === "prov" ? <span style={{ fontSize: 10, color: r.editadoManual ? "#fb923c" : "#fbbf24" }}>{r.editadoManual ? "editado" : "auto"}</span> : <span style={{ fontSize: 10, color: f.aprobado ? "#22c55e" : "#475569" }}>{f.aprobado ? "🔒" : ""}</span>}</td></tr>))}
              </tbody>
              <tfoot><tr style={{ background: "#131c2e", borderTop: "2px solid rgba(255,255,255,0.08)" }}><td colSpan={3} style={{ padding: "6px 9px", color: "#64748b", fontSize: 11, fontWeight: 600 }}>TOTALES</td><td style={{ padding: "6px 9px", textAlign: "right", fontWeight: 700, color: "#4ade80", fontSize: 12 }}>{fmt(totalDeb)}</td><td style={{ padding: "6px 9px", textAlign: "right", fontWeight: 700, color: "#f87171", fontSize: 12 }}>{fmt(totalCre)}</td><td style={{ padding: "6px 9px", textAlign: "center" }}><span style={{ fontWeight: 700, color: cuadra ? "#22c55e" : "#f87171" }}>{cuadra ? "✓" : "✗"}</span></td></tr></tfoot>
            </table>
          </div>
...
function ModalTratamiento({ archivos, empresaActual, empresas, onEmpresa, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.88)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}>
      <div style={{ background: "#1a2035", border: "1px solid #232840", borderRadius: 16, padding: 26, maxWidth: 540, width: "100%" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}><div style={{ fontSize: 26, marginBottom: 8 }}>📋</div><div style={{ fontFamily: "sans-serif", fontWeight: 700, fontSize: 16, color: "#fff", marginBottom: 4 }}>¿Cómo se contabiliza?</div><div style={{ fontSize: 11, color: "#64748b" }}>{archivos.length === 1 ? `📄 ${archivos[0].name}` : `${archivos.length} archivos`}</div></div>
        {!empresaActual && empresas.length > 0 && (<div style={{ background: "#1e2a3a", border: "1px solid #4f7cff", borderRadius: 8, padding: "10px 14px", marginBottom: 16 }}><div style={{ fontSize: 11, color: "#60a5fa", marginBottom: 6, fontWeight: 600 }}>🏢 Selecciona la empresa</div><select defaultValue="" onChange={e => { const emp = empresas.find(x => x.nit === e.target.value); if (emp) onEmpresa(emp); }} style={{ width: "100%", background: "#131c2e", border: "1px solid #2d3352", color: "#e8eaf0", borderRadius: 6, padding: "7px 10px", fontSize: 12, cursor: "pointer", outline: "none" }}><option value="" disabled>— elige empresa —</option>{empresas.map(e => <option key={e.nit} value={e.nit}>{e.nombre} · {e.nit}</option>)}</select></div>)}
        ...
        <div style={{ marginBottom: 16 }}><div style={{ fontSize: 10, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>Paso 1 — Tratamiento</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>{[{ key: "inventario", icono: "📦", titulo: "Inventario", desc: "Ítem por ítem · cuentas 1x", color: "#1e3a5f", borde: "#3b6fd4" }, { key: "gasto", icono: "📉", titulo: "Costo / Gasto", desc: "Concepto resumido · cuentas 6x/5x", color: "#2d1b4e", borde: "#8b5cf6" }].map(op => (<button key={op.key} onClick={() => setTratamiento(op.key)} style={{ background: tratamiento === op.key ? op.color : "#131c2e", border: `2px solid ${tratamiento === op.key ? op.borde : "#232840"}`, borderRadius: 10, padding: "13px", cursor: "pointer", textAlign: "left" }}><div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}><span style={{ fontSize: 17 }}>{op.icono}</span><span style={{ fontFamily: "sans-serif", fontWeight: 700, fontSize: 13, color: "#fff" }}>{op.titulo}</span>{tratamiento === op.key && <span style={{ marginLeft: "auto", color: op.borde }}>✓</span>}</div><div style={{ fontSize: 11, color: "#94a3b8" }}>{op.desc}</div></button>))}</div></div>
        <div style={{ marginBottom: 16 }}><div style={{ fontSize: 10, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>Paso 2 — IVA</div><div style={{ display: "flex", flexDirection: "column", gap: 7 }}>{[{ key: "descontable", icono: "🔄", titulo: "IVA descontable", desc: "Activo · cuentas 24x", color: "#1e3a5f", borde: "#3b6fd4" }, { key: "gasto", icono: "📉", titulo: "IVA al gasto (consorcio)", desc: "Cuentas 61157001 / 61157002", color: "#1a2d1a", borde: "#22c55e" }].map(op => (<button key={op.key} onClick={() => setTratIva(op.key)} style={{ background: tratIva === op.key ? op.color : "#131c2e", border: `2px solid ${tratIva === op.key ? op.borde : "#232840"}`, borderRadius: 9, padding: "10px 13px", cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 17, minWidth: 22 }}>{op.icono}</span><div style={{ flex: 1 }}><div style={{ fontFamily: "sans-serif", fontWeight: 700, fontSize: 13, color: "#fff", marginBottom: 2 }}>{op.titulo}</div><div style={{ fontSize: 11, color: "#94a3b8" }}>{op.desc}</div></div>{tratIva === op.key && <span style={{ fontSize: 14, color: op.borde }}>✓</span>}</button>))}</div></div>
        <div style={{ background: "#0a1a0a", border: "1px solid #166534", borderRadius: 6, padding: "6px 12px", marginBottom: 14, fontSize: 11, color: "#4ade80" }}>✓ <strong>Retenciones calculadas por código</strong> — no por IA.</div>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}><button onClick={onCancel} style={{ background: "transparent", border: "1px solid #2d3352", color: "#94a3b8", padding: "8px 16px", borderRadius: 6, cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Cancelar</button><button onClick={() => listo && onConfirm(tratamiento, tratIva)} disabled={!listo} style={{ background: listo ? "#4f7cff" : "rgba(255,255,255,0.08)", color: listo ? "#fff" : "#475569", border: "none", padding: "8px 22px", borderRadius: 6, cursor: listo ? "pointer" : "not-allowed", fontSize: 13, fontWeight: 700 }}>{listo ? "Procesar →" : !empresaActual ? "Selecciona empresa primero" : "Completa los 2 pasos"}</button></div>
      </div>
    </div>
  );
}
...
function ModalExport({ facturas, onClose, pucCuentas = [] }) {
  ...
  const errores = [];
  aprobadas.forEach(f => { (f.asiento||[]).forEach(r => { if (!r.cuenta) { errores.push(`${f.razonSocial}: línea sin cuenta`); return; } if (pucCuentas.length > 0 && !pucCuentas.find(c => c.codigo === r.cuenta)) errores.push(`${f.razonSocial}: cuenta ${r.cuenta} no existe en PUC`); }); const deb = (f.asiento||[]).filter(r=>r.tipo==="debito").reduce((s,r)=>s+r.valor,0); const cre = (f.asiento||[]).filter(r=>r.tipo==="credito").reduce((s,r)=>s+r.valor,0); if (Math.abs(deb-cre)>1) errores.push(`${f.razonSocial}: asiento descuadrado`); });
  ...
  const exportar = (soloUna = null) => { const lista = soloUna ? [soloUna] : aprobadas; const wsData = [headers]; lista.forEach((f, fi) => { const docNum = (parseInt(cfg.docNumInicio) || 1) + fi; (f.asiento || []).forEach(r => { wsData.push([docNum, f.fecha || "", cfg.tpcCod, r.cuenta, r.descripcion, f.nitProveedor || "", cfg.ctoCod, r.tipo === "debito" ? r.valor : "", r.tipo === "credito" ? r.valor : "", cfg.prfCod, cfg.docAux, ""]); }); }); const ws = XLSX.utils.aoa_to_sheet(wsData); ws["!cols"] = [{wch:8},{wch:12},{wch:8},{wch:12},{wch:45},{wch:14},{wch:10},{wch:14},{wch:14},{wch:10},{wch:14},{wch:8}]; const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Comprobante"); XLSX.writeFile(wb, soloUna ? `comprobante_${soloUna.prefijo || soloUna.nitProveedor}.xlsx` : `comprobante_${cfg.tpcCod}_${cfg.docNumInicio}.xlsx`); };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.88)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}>
      <div style={{ background: "#1a2035", border: "1px solid #232840", borderRadius: 16, padding: 26, maxWidth: 700, width: "100%" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}><div><div style={{ fontFamily: "sans-serif", fontWeight: 700, fontSize: 16, color: "#fff" }}>⬇ Exportar comprobante contable</div><div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>{aprobadas.length} facturas aprobadas</div></div><button onClick={onClose} style={{ background: "transparent", border: "1px solid #2d3352", color: "#94a3b8", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}>✕</button></div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 11, marginBottom: 18 }}>{[{ k: "docNumInicio", label: "DocNum inicial", ph: "1" }, { k: "tpcCod", label: "TpcCod", ph: "CO" }, { k: "prfCod", label: "PrfCod", ph: "COMP" }, { k: "ctoCod", label: "Centro de costo", ph: "CC001" }, { k: "docAux", label: "DocAux", ph: "OC-2026" }].map(({ k, label, ph }) => (<div key={k}><div style={{ fontSize: 10, color: "#64748b", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 3 }}>{label}</div><input value={cfg[k]} onChange={e => set(k, e.target.value)} placeholder={ph} style={{ width: "100%", background: "#131c2e", border: "1px solid #2d3352", color: "#e8eaf0", borderRadius: 6, padding: "7px 10px", fontFamily: "monospace", fontSize: 12, outline: "none" }} /></div>))}</div>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", flexWrap: "wrap" }}>
          {errores.length > 0 && (<div style={{background:"#1a0a0a",border:"1px solid #7c3700",borderRadius:8,padding:"10px 14px",flex:1}}><div style={{fontSize:11,color:"#f87171",fontWeight:600,marginBottom:6}}>⚠ {errores.length} problema(s):</div>{errores.map((e,i)=><div key={i} style={{fontSize:10,color:"#fb923c",marginBottom:2}}>• {e}</div>)}</div>)}
          {errores.length === 0 && aprobadas.length > 0 && (<div style={{background:"#0a1a0a",border:"1px solid #166534",borderRadius:8,padding:"8px 14px",fontSize:11,color:"#4ade80",flex:1}}>✓ Todo verificado — {aprobadas.length} facturas listas</div>)}
          <button onClick={() => { if(errores.length>0&&!confirm(`Hay ${errores.length} problema(s). ¿Exportar?`)) return; exportar(); }} disabled={aprobadas.length === 0} style={{ background: aprobadas.length ? (errores.length>0?"#f59e0b":"#4f7cff") : "rgba(255,255,255,0.08)", color: aprobadas.length ? "#fff" : "#475569", border: "none", borderRadius: 8, padding: "10px 22px", cursor: aprobadas.length ? "pointer" : "not-allowed", fontSize: 13, fontWeight: 700 }}>⬇ Exportar ({aprobadas.length})</button>
        </div>
      </div>
    </div>
  );
...
function LoginScreen({ onLogin }) {
  ...
  const intentar = async () => { if (!usuario || !clave) { setError("Ingresa usuario y clave"); return; } const rows = await sbGet("empresas", { usuario }); const found = rows.find(r => r.clave === clave); if (found) { onLogin(found); return; } if (usuario === "admin" && clave === "contai2026") { onLogin({ usuario: "admin", nombre: "Administrador", id: "admin" }); return; } setError("Usuario o clave incorrectos"); };
  return (<div style={{ fontFamily: "'Inter',system-ui,sans-serif", background: "#0f1f3d", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}><div style={{ background: "#1a2035", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 36, maxWidth: 380, width: "100%" }}><div style={{ textAlign: "center", marginBottom: 28 }}><div style={{ width: 52, height: 52, background: "linear-gradient(135deg,#1a3a6e,#2a5298)", borderRadius: 14, display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 14, boxShadow: "0 4px 18px rgba(79,142,247,0.35)" }}><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#4f8ef7" strokeWidth="2.2" strokeLinecap="round"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="16" y2="12"/><line x1="3" y1="18" x2="12" y2="18"/></svg></div><div style={{ fontFamily: "'Inter',sans-serif", fontWeight: 800, fontSize: 22, color: "#fff", letterSpacing: "-0.4px" }}>Conta<span style={{ color: "#4f8ef7" }}>IA</span></div><div style={{ fontSize: 10, color: "#64748b", marginTop: 6, letterSpacing: "0.12em", textTransform: "uppercase" }}>Contabiliza · Valida · Sincroniza</div></div><div style={{ display: "flex", flexDirection: "column", gap: 14 }}><div><div style={{ fontSize: 10, color: "#64748b", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 5 }}>Usuario</div><input value={usuario} onChange={e => setUsuario(e.target.value)} onKeyDown={e => e.key === "Enter" && intentar()} placeholder="admin" autoFocus style={{ width: "100%", background: "#131c2e", border: "1px solid rgba(255,255,255,0.1)", color: "#e8eaf0", borderRadius: 8, padding: "10px 14px", fontFamily: "monospace", fontSize: 13, outline: "none", boxSizing: "border-box" }} /></div><div><div style={{ fontSize: 10, color: "#64748b", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 5 }}>Clave</div><input type="password" value={clave} onChange={e => setClave(e.target.value)} onKeyDown={e => e.key === "Enter" && intentar()} placeholder="••••••••" style={{ width: "100%", background: "#131c2e", border: "1px solid rgba(255,255,255,0.1)", color: "#e8eaf0", borderRadius: 8, padding: "10px 14px", fontFamily: "monospace", fontSize: 13, outline: "none", boxSizing: "border-box" }} /></div>{error && <div style={{ background: "#1a0a0a", border: "1px solid #3b1f1f", borderRadius: 6, padding: "8px 12px", fontSize: 12, color: "#f87171" }}>⚠ {error}</div>}<button onClick={intentar} style={{ background: "#4f8ef7", color: "#fff", border: "none", borderRadius: 8, padding: "11px", cursor: "pointer", fontSize: 14, fontWeight: 700, marginTop: 4 }}>Ingresar →</button></div><div style={{ marginTop: 20, fontSize: 10, color: "#475569", textAlign: "center" }}>Supabase · Multi-empresa · ContaFlex</div></div></div>);
}
...
function PanelCorreccion({ facturas, pucCuentas, onUpdate, onClose }) {
  const cuentasUsadas = {};
  facturas.filter(f => !f.error && f.asiento).forEach(f => { (f.asiento || []).forEach(r => { if (!r.cuenta || r.id === "prov") return; if (!cuentasUsadas[r.cuenta]) { const enPuc = pucCuentas.find(c => c.codigo === r.cuenta); cuentasUsadas[r.cuenta] = { codigo: r.cuenta, descripcion: r.descripcion, count: 0, enPuc: !!enPuc, nombrePuc: enPuc?.nombre || "", facturas: [] }; } cuentasUsadas[r.cuenta].count++; if (!cuentasUsadas[r.cuenta].facturas.includes(f.id)) cuentasUsadas[r.cuenta].facturas.push(f.id); }); });
  const lista = Object.values(cuentasUsadas).sort((a,b) => (a.enPuc===b.enPuc)?0:a.enPuc?1:-1);
  const [correcciones, setCorrecciones] = useState({});
  const aplicarCorrecciones = () => { Object.entries(correcciones).forEach(([cuentaVieja, cuentaNueva]) => { if (!cuentaNueva || cuentaNueva === cuentaVieja) return; const enPuc = pucCuentas.find(c => c.codigo === cuentaNueva); facturas.forEach(f => { if (!f.asiento) return; const nuevasFila = f.asiento.map(r => r.cuenta === cuentaVieja ? { ...r, cuenta: cuentaNueva, descripcion: enPuc?.nombre || r.descripcion, advertencia: !enPuc } : r); if (JSON.stringify(nuevasFila) !== JSON.stringify(f.asiento)) onUpdate(f.id, "asiento", nuevasFila); }); }); onClose(); };
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div style={{background:"#1a2035",border:"1px solid #232840",borderRadius:16,maxWidth:800,width:"100%",maxHeight:"90vh",display:"flex",flexDirection:"column"}}><div style={{padding:"14px 20px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:12}}><div style={{fontFamily:"sans-serif",fontWeight:700,fontSize:15,color:"#fff"}}>🔧 Corrección masiva de cuentas</div><span style={{fontSize:11,color:"#64748b"}}>{lista.length} cuentas</span><div style={{marginLeft:"auto",display:"flex",gap:8}}><button onClick={aplicarCorrecciones} style={{background:"#4f7cff",color:"#fff",border:"none",borderRadius:6,padding:"6px 16px",cursor:"pointer",fontSize:12,fontWeight:700}}>✓ Aplicar</button><button onClick={onClose} style={{background:"transparent",border:"1px solid #2d3352",color:"#94a3b8",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:12}}>✕</button></div></div><div style={{flex:1,overflowY:"auto",padding:"12px 20px"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{background:"#131c2e"}}>{["Cuenta","Nombre PUC","¿En PUC?","Corregir a"].map(h=><th key={h} style={{padding:"7px 10px",textAlign:"left",color:"#475569",fontSize:10,fontWeight:600,borderBottom:"1px solid rgba(255,255,255,0.08)"}}>{h}</th>)}</tr></thead><tbody>{lista.map(c=>(<tr key={c.codigo} style={{borderBottom:"1px solid #1a1d27",background:!c.enPuc?"#1a0a0a":"transparent"}}><td style={{padding:"6px 10px",fontFamily:"monospace",color:c.enPuc?"#60a5fa":"#f87171",fontWeight:600}}>{c.codigo}</td><td style={{padding:"6px 10px",color:"#94a3b8"}}>{c.nombrePuc||c.descripcion}</td><td style={{padding:"6px 10px",textAlign:"center"}}>{c.enPuc?<span style={{color:"#22c55e"}}>✓</span>:<span style={{color:"#f87171"}}>✗</span>}</td><td style={{padding:"6px 10px"}}><select onChange={e=>setCorrecciones(p=>({...p,[c.codigo]:e.target.value}))} style={{background:"#131c2e",border:`1px solid ${!c.enPuc?"#f87171":"#2d3352"}`,color:"#e8eaf0",borderRadius:5,padding:"3px 7px",fontSize:11,width:"100%",cursor:"pointer",outline:"none"}}><option value="">— sin cambio —</option>{pucCuentas.filter(p=>p.codigo.startsWith(c.codigo.slice(0,2))).map(p=><option key={p.codigo} value={p.codigo}>{p.codigo} — {p.nombre}</option>)}</select></td></tr>))}</tbody></table></div></div></div>);
}
...
async function enviarAContaFlex(facturas, empresaActual, tipoCod = "CO") {
  ...
  const aprobadas = [...facturas].filter(f => f.aprobado && !f.error && f.asiento).sort((a,b) => (a.fecha||"").localeCompare(b.fecha||""));
  if (aprobadas.length === 0) throw new Error("No hay facturas aprobadas para enviar");
  const porAnno = {};
  aprobadas.forEach(f => { const anno = (f.fecha||"2026").slice(0,4); if (!porAnno[anno]) porAnno[anno] = []; porAnno[anno].push(f); });
  const resultados = []; let errores = [];
  for (const [anno, facts] of Object.entries(porAnno)) {
    ...
    let ultimoNum = 0;
    try { const r = await fetch(url, { headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}` } }); const rows = await r.json(); if (rows && rows.length > 0) { const n = parseInt(String(rows[0].numero).replace(/[^0-9]/g,"")) || 0; ultimoNum = n; } } catch(e) {}
    for (const f of facts) {
      ultimoNum++;
      const nit = (f.nitProveedor||"").replace(/[^0-9]/g,"");
      const subtotal = f.subtotal || 0; const iva = f.totalIva || 0; const rete = f.retefuente || 0; const reteica = f.retica || 0; const neto = (f.total||0) - rete - reteica; const retePct = f.rete?.pct || f.reteInfo?.pct || 0; const reteBase = rete > 0 ? subtotal : 0;
      const filaRete = (f.asiento||[]).find(r => r.id === "rete"); const filaIva = (f.asiento||[]).find(r => r.id === "iva");
      const numFactura = f.prefijo || f.numeroFactura || f.nroDocumento || "";
      const detallesJson = (f.asiento||[]).map(r => ({ id: generarId(), cuenta: r.cuenta || "", detalle: r.descripcion || "", documento: numFactura, tercero_nit: nit, tercero_nombre: f.razonSocial || "", debito: r.tipo === "debito" ? r.valor : 0, credito: r.tipo === "credito" ? r.valor : 0, base_gravable: r.id === "rete" ? (f.subtotal || 0) : 0, tarifa: r.id === "rete" ? (f.rete?.pct || 0) : 0 }));
      const comprobante = { id: generarId(), empresa_id: empresaActual.id, tipo_doc: tipoCod, numero: String(ultimoNum), fecha: f.fecha || new Date().toISOString().slice(0,10), tercero_nit: nit, tercero_nombre: f.razonSocial || "", concepto: f.ia?.concepto_general || f.razonSocial || "", estado: "activo", valor_bruto: subtotal, base_retefuente: reteBase, tarifa_retefuente: retePct, valor_retefuente: rete, cuenta_retefuente: filaRete?.cuenta || "", base_iva: iva > 0 ? subtotal : 0, tarifa_iva: iva > 0 ? 19 : 0, valor_iva: iva, cuenta_iva: filaIva?.cuenta || "", base_reteiva: 0, tarifa_reteiva: 0, valor_reteiva: 0, cuenta_reteiva: "", base_reteica: reteica > 0 ? subtotal : 0, tarifa_reteica: 0, valor_reteica: reteica, cuenta_reteica: "", neto_pagar: neto, detalles: JSON.stringify(detallesJson) };
      const compId = comprobante.id;
      const rComp = await fetch(`${SB_URL}/rest/v1/comprobantes`, { method: "POST", headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}`, "Content-Type": "application/json", Prefer: "return=minimal" }, body: JSON.stringify(comprobante) });
      if (!rComp.ok) { const err = await rComp.text(); errores.push(`${f.razonSocial}: ${err}`); continue; }
...
function ModalEnviar({ facturas, empresaActual, onClose, onEnviado }) {
  const aprobadas = facturas.filter(f => f.aprobado && !f.error && f.asiento).sort((a,b) => (a.fecha||"").localeCompare(b.fecha||""));
  const [tipoCod, setTipoCod] = useState("CO"); const [enviando, setEnviando] = useState(false); const [resultado, setResultado] = useState(null);
  const fmt = n => `$${Number(n||0).toLocaleString("es-CO")}`;
  const enviar = async () => { setEnviando(true); try { const res = await enviarAContaFlex(facturas, empresaActual, tipoCod); setResultado(res); if (res.resultados.length > 0) onEnviado(res.resultados); } catch(e) { setResultado({ resultados:[], errores:[e.message] }); } setEnviando(false); };
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div style={{background:"#1a2035",border:"1px solid #232840",borderRadius:16,maxWidth:700,width:"100%",maxHeight:"90vh",display:"flex",flexDirection:"column"}}><div style={{padding:"14px 20px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:12}}><div style={{fontFamily:"sans-serif",fontWeight:700,fontSize:15,color:"#fff"}}>🚀 Enviar a ContaFlex</div><span style={{fontSize:11,color:"#64748b"}}>{aprobadas.length} aprobadas</span><button onClick={onClose} style={{marginLeft:"auto",background:"transparent",border:"1px solid #2d3352",color:"#94a3b8",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:12}}>✕</button></div><div style={{flex:1,overflowY:"auto",padding:"16px 20px"}}>{!resultado?(<><div style={{background:"#0a1a0a",border:"1px solid #166534",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#4ade80"}}>✓ Se insertarán en comprobantes con consecutivo automático.</div><div style={{marginBottom:16}}><div style={{fontSize:10,color:"#64748b",fontWeight:600,textTransform:"uppercase",letterSpacing:".07em",marginBottom:6}}>Tipo de comprobante</div><div style={{display:"flex",gap:8}}>{["CO","FC","CE"].map(t=>(<button key={t} onClick={()=>setTipoCod(t)} style={{background:tipoCod===t?"#1e2a3a":"#131c2e",border:`2px solid ${tipoCod===t?"#4f7cff":"#232840"}`,color:tipoCod===t?"#60a5fa":"#64748b",borderRadius:8,padding:"8px 20px",cursor:"pointer",fontSize:13,fontWeight:700}}>{t}</button>))}</div></div><div style={{background:"#0f1a2e",border:"1px solid #1e3a5f",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#60a5fa"}}><strong>Empresa:</strong> {empresaActual?.nombre} · {empresaActual?.nit}</div></>):(<div>{resultado.resultados.length>0&&(<div style={{background:"#0a1a0a",border:"1px solid #166534",borderRadius:8,padding:"12px 16px",marginBottom:12}}><div style={{fontSize:13,fontWeight:700,color:"#4ade80",marginBottom:8}}>✓ {resultado.resultados.length} enviadas</div>{resultado.resultados.map(f=>(<div key={f.id} style={{fontSize:11,color:"#94a3b8",marginBottom:3}}>{tipoCod}-{String(f.compNumero).padStart(4,"0")} · {f.fecha} · {f.razonSocial}</div>))}</div>)}{resultado.errores.length>0&&(<div style={{background:"#1a0a0a",border:"1px solid #7c3700",borderRadius:8,padding:"12px 16px"}}><div style={{fontSize:13,fontWeight:700,color:"#f87171",marginBottom:8}}>✗ {resultado.errores.length} errores</div>{resultado.errores.map((e,i)=><div key={i} style={{fontSize:11,color:"#fb923c",marginBottom:3}}>• {e}</div>)}</div>)}</div>)}</div><div style={{padding:"12px 20px",borderTop:"1px solid rgba(255,255,255,0.08)",display:"flex",gap:10,justifyContent:"flex-end"}}>{!resultado?(<><button onClick={onClose} style={{background:"transparent",border:"1px solid #2d3352",color:"#94a3b8",padding:"8px 16px",borderRadius:6,cursor:"pointer",fontSize:13,fontWeight:600}}>Cancelar</button><button onClick={enviar} disabled={enviando||aprobadas.length===0} style={{background:enviando?"rgba(255,255,255,0.08)":"#22c55e",color:"#fff",border:"none",borderRadius:6,padding:"8px 22px",cursor:enviando?"not-allowed":"pointer",fontSize:13,fontWeight:700}}>{enviando?"Enviando...":`🚀 Enviar ${aprobadas.length}`}</button></>):(<button onClick={onClose} style={{background:"#4f7cff",color:"#fff",border:"none",borderRadius:6,padding:"8px 22px",cursor:"pointer",fontSize:13,fontWeight:700}}>Cerrar</button>)}</div></div></div>);
}



function PanelAuditoriaCuentas({ facturas, onClose, onUpdate, pucCuentas }) {
  const [cuentaSel, setCuentaSel] = useState(null);
  const fmt = n => "$" + Number(n||0).toLocaleString("es-CO");
  const cuentaMap = {};
  facturas.filter(f => f.asiento).forEach(f => { (f.asiento||[]).forEach(r => { if (!r.cuenta) return; if (!cuentaMap[r.cuenta]) cuentaMap[r.cuenta] = { cuenta: r.cuenta, nombre: r.descripcion || r.cuenta, debito: 0, credito: 0, facturas: [] }; cuentaMap[r.cuenta].debito += r.tipo === "debito" ? (r.valor||0) : 0; cuentaMap[r.cuenta].credito += r.tipo === "credito" ? (r.valor||0) : 0; if (!cuentaMap[r.cuenta].facturas.find(x => x.id === f.id)) cuentaMap[r.cuenta].facturas.push(f); }); });
  const cuentas = Object.values(cuentaMap).sort((a,b) => a.cuenta.localeCompare(b.cuenta));
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div style={{background:"#1a2035",border:"1px solid #232840",borderRadius:16,width:"100%",maxWidth:1000,maxHeight:"92vh",display:"flex",flexDirection:"column"}}><div style={{padding:"14px 20px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:12}}><div style={{fontWeight:700,fontSize:15,color:"#fff"}}>🔍 Auditoría por cuenta</div><button onClick={onClose} style={{marginLeft:"auto",background:"transparent",border:"1px solid #2d3352",color:"#94a3b8",borderRadius:6,padding:"4px 12px",cursor:"pointer"}}>✕ Cerrar</button></div><div style={{flex:1,overflow:"hidden",display:"grid",gridTemplateColumns:cuentaSel?"1fr 1fr":"1fr"}}><div style={{overflowY:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{background:"#131c2e",position:"sticky",top:0}}>{["Cuenta","Nombre","Débito","Crédito","Facts"].map(h=>(<th key={h} style={{padding:"8px 12px",textAlign:["Débito","Crédito","Facts"].includes(h)?"right":"left",fontSize:10,fontWeight:700,color:"#475569",textTransform:"uppercase",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>{h}</th>))}</tr></thead><tbody>{cuentas.map((ct,i)=>(<tr key={ct.cuenta} onClick={()=>setCuentaSel(cuentaSel===ct.cuenta?null:ct.cuenta)} style={{borderBottom:"1px solid rgba(30,34,58,.5)",cursor:"pointer",background:cuentaSel===ct.cuenta?"rgba(79,142,247,.12)":i%2===0?"transparent":"rgba(255,255,255,.01)"}}><td style={{padding:"8px 12px",fontFamily:"monospace",color:"#e8b84b",fontWeight:600}}>{ct.cuenta}</td><td style={{padding:"8px 12px",color:"#94a3b8",maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ct.nombre}</td><td style={{padding:"8px 12px",textAlign:"right",fontFamily:"monospace",color:"#4ade80",fontSize:11}}>{ct.debito>0?fmt(ct.debito):"—"}</td><td style={{padding:"8px 12px",textAlign:"right",fontFamily:"monospace",color:"#f87171",fontSize:11}}>{ct.credito>0?fmt(ct.credito):"—"}</td><td style={{padding:"8px 12px",textAlign:"right",color:"#64748b",fontSize:11}}>{ct.facturas.length}</td></tr>))}</tbody></table></div>{cuentaSel&&(<div style={{overflowY:"auto"}}><div style={{padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,0.08)",background:"#131c2e",position:"sticky",top:0}}><div style={{fontSize:12,fontWeight:700,color:"#4f8ef7"}}>Cuenta {cuentaSel}</div></div>{(cuentaMap[cuentaSel]?.facturas||[]).map(f=>(<div key={f.id} style={{padding:"12px 14px",borderBottom:"1px solid rgba(30,34,58,.5)"}}><div style={{fontWeight:600,color:"#e8eaf0",fontSize:12}}>{f.razonSocial}</div><div style={{fontSize:11,color:"#64748b"}}>NIT {f.nitProveedor} · {f.fecha} · <span style={{color:"#4ade80"}}>{fmt(f.total)}</span></div></div>))}</div>)}</div></div></div>);
}
...
export default function App() {
  ...
  const [modalDIAN, setModalDIAN] = useState(false);
  ...
  useEffect(() => { if (!empresaActual?.id) return; sbGet("terceros", { empresa_id: empresaActual.id }).then(t => setTercerosList(t || [])); }, [empresaActual?.id]);

  if (!usuarioActual) return <LoginScreen onLogin={setUsuarioActual} />;

  const recibirArchivos = (lista) => { const v = Array.from(lista).filter(f => f.name.endsWith(".xml") || f.name.endsWith(".pdf")); if (v.length) setModal({ archivos: v }); };

  const confirmarTratamiento = async (tratamiento, tratIva) => {
    const { archivos } = modal;
    setModal(null);
    const placeholders = archivos.map((archivo, i) => ({ id: Date.now() + i + Math.random(), fechaCarga: new Date().toISOString(), archivo: archivo.name, tratamiento, tratIva, procesando: true, aprobado: false, asiento: null }));
        ...
        try {
          ...
          if (archivo.name.toLowerCase().endsWith(".xml")) {
            ...
            if (datosPreview.prefijo && datosPreview.nitProveedor) {
              ...
              const dup = await sbGet("dian_facturas", { prefijo: datosPreview.prefijo, nit_proveedor: nit_prev, empresa_nit: empresaActual?.nit || "" });
              if (dup && dup.length > 0) { const d = dup[0]; const diasAtras = Math.floor((Date.now() - new Date(d.fecha_carga)) / 86400000); setFacturas(prev => [...prev, { id: Date.now()+Math.random(), fechaCarga: new Date().toISOString(), archivo: archivo.name, error: `DUPLICADO — contabilizada hace ${diasAtras} días. Proveedor: ${d.razon_social}.` }]); return; }
            }
          }
    ...
    <div style={{ fontFamily: "'Inter',system-ui,sans-serif", background: "#0f1f3d", color: "#e8eaf0", display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
      ...
      {modalDIAN && (
        <ModalDescargaDIAN
          ...
          onXmlsDescargados={(xmlFiles) => {
            const files = xmlFiles.map(x => new File([x.contenido], x.nombre, { type: "text/xml" }));
            setModalDIAN(false);
            setModal({ archivos: files });
          }}
        />
      )}

      {modalAuditoria && <PanelAuditoriaCuentas facturas={[...facturas, ...aprobadasAcum, ...subidas]} onClose={() => setModalAuditoria(false)} onUpdate={upd} pucCuentas={pucCuentas} />}
      {modalExport && <ModalExport facturas={facturas} onClose={() => setModalExport(false)} pucCuentas={pucCuentas} />}
      {modalEnviar && <ModalEnviar facturas={facturas} empresaActual={empresaActual} onClose={() => setModalEnviar(false)} onEnviado={(res) => { setModalEnviar(false); const enviadas = facturas.filter(f => f.aprobado && !f.error && f.asiento).map(f => ({ ...f, fechaSubida: new Date().toISOString().slice(0,10) })); guardarSubidas([...subidas, ...enviadas]); guardarAprobadasAcum([]); setFacturas(prev => prev.filter(f => !f.aprobado || f.error)); res.forEach(f => upd(f.id, "enviado", true)); }} />}
      ...
      {modalTerceros && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:3000,display:"flex",flexDirection:"column",padding:16}}>
          <div style={{background:"#1a2035",border:"1px solid #232840",borderRadius:16,flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
            <div style={{padding:"14px 20px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:12}}>
              ...
              <button onClick={() => { const headers = ["tipo_doc","numero","digito_verificacion","razon_social","nombre","direccion","telefono","celular","email","ciudad","departamento","pais","persona","regimen","es_cliente","es_proveedor","es_empleado","gran_contribuyente","autoretenedor","agente_retencion_iva","del_exterior"]; const ws = XLSX.utils.aoa_to_sheet([headers,...tercerosList.map(t=>headers.map(h=>t[h]||""))]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,"Terceros"); XLSX.writeFile(wb,`Terceros_${new Date().toISOString().slice(0,10)}.xlsx`); }} style={{background:"#166534",color:"#fff",border:"none",borderRadius:6,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:600,marginLeft:"auto"}}>⬇ Excel</button>
            ...
            <div style={{flex:1,overflowX:"auto",overflowY:"auto"}}>
              {tercerosList.length === 0 ? <div style={{padding:40,textAlign:"center",color:"#475569"}}>Sin terceros aún</div> : <table style={{borderCollapse:"collapse",fontSize:11,minWidth:"100%"}}><thead><tr style={{background:"#131c2e",position:"sticky",top:0}}>{["NIT","DV","Razón Social","Teléfono","Email","Ciudad","Persona","Régimen","AutoRet"].map(h=><th key={h} style={{padding:"7px 10px",textAlign:"left",color:"#475569",fontSize:10,fontWeight:600,borderBottom:"1px solid rgba(255,255,255,0.08)",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{tercerosList.map((t,i)=>(<tr key={t.numero} style={{borderBottom:"1px solid #1a1d27",background:i%2===0?"transparent":"#0f1f3d"}}><td style={{padding:"5px 10px",fontFamily:"monospace",color:"#60a5fa"}}>{t.numero}</td><td style={{padding:"5px 10px",color:"#64748b"}}>{t.digito_verificacion}</td><td style={{padding:"5px 10px",color:"#e8eaf0",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.razon_social}</td><td style={{padding:"5px 10px",color:"#94a3b8"}}>{t.telefono}</td><td style={{padding:"5px 10px",color:"#94a3b8",maxWidth:140,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.email}</td><td style={{padding:"5px 10px",color:"#94a3b8"}}>{t.ciudad}</td><td style={{padding:"5px 10px",color:t.persona==="Jurídica"?"#60a5fa":"#fb923c"}}>{t.persona}</td><td style={{padding:"5px 10px",color:"#64748b",maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.regimen}</td><td style={{padding:"5px 10px",textAlign:"center"}}>{t.autoretenedor?<span style={{color:"#fb923c",fontWeight:700}}>🔒</span>:<span style={{color:"#2d3352"}}>—</span>}</td></tr>))}</tbody></table>}
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ background: "#0d1a33", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "10px 20px", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1, maxWidth: 360, margin: "0 12px" }}>
          ...
          {empresas.length === 0 ? <span style={{ fontSize: 11, color: "#f87171" }}>Sin empresas en Supabase</span> : <select value={empresaActual?.nit || ""} onChange={e => { const emp = empresas.find(x => x.nit === e.target.value); if (emp) setEmpresaActual(emp); }} style={{ flex: 1, background: "#131c2e", border: "1px solid #2d3352", color: "#e8eaf0", borderRadius: 6, padding: "5px 8px", fontSize: 11, cursor: "pointer", outline: "none" }}>{empresas.map(e => <option key={e.nit} value={e.nit}>{e.nombre} · {e.nit}</option>)}</select>}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginLeft: "auto" }}>
          ...
          <button onClick={() => setModalDIAN(true)} style={{ background: "linear-gradient(135deg,#0066ff,#00e5a0)", color: "#000", border: "none", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>📥 DIAN</button>
          ...
          {facturas.filter(f => !f.error && f.asiento).length > 0 && <button onClick={() => setModalCorreccion(true)} style={{ background: "transparent", border: "1px solid #f59e0b", color: "#f59e0b", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>🔧 Cuentas</button>}
          {facturas.filter(f => !f.aprobado && !f.error && f.asiento).length > 0 && <button onClick={() => { facturas.filter(f => !f.aprobado && !f.error && f.asiento).forEach(f => upd(f.id, "aprobado", true)); }} style={{ background: "#4f7cff", color: "#fff", border: "none", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>✓ Aprobar todas</button>}
          {(aprobadas.length > 0 || aprobadasAcum.length > 0) && <button onClick={() => setModalAuditoria(true)} style={{ background:"transparent",border:"1px solid #4f8ef7",color:"#4f8ef7",borderRadius:6,padding:"5px 12px",cursor:"pointer",fontSize:12,fontWeight:600 }}>🔍 Auditoría</button>}
          ...
          {aprobadas.length > 0 && <button onClick={() => setFacturas(prev => prev.filter(f => !f.aprobado))} style={{ background: "transparent", border: "1px solid #166534", color: "#4ade80", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>🗑 Aprobadas</button>}
          {facturas.some(f => f.enviado) && <button onClick={() => { if(confirm("¿Eliminar enviadas?")) setFacturas(p => p.filter(f => !f.enviado)); }} style={{ background: "transparent", border: "1px solid #166534", color: "#4ade80", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>🗑 Enviadas</button>}
          {facturas.length > 0 && <button onClick={limpiar} style={{ background: "transparent", border: "1px solid #2d3352", color: "#64748b", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11 }}>🗑 Todo</button>}
          <button onClick={() => setModalTerceros(true)} style={{ background: "transparent", border: "1px solid #2d3352", color: "#60a5fa", borderRadius: 6, padding: "4px 9px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>👥</button>
          <button onClick={() => { if (window.confirm("¿Limpiar TODO?")) { limpiar(); guardarSubidas([]); guardarAprobadasAcum([]); } }} style={{ background: "transparent", border: "1px solid #7c2020", color: "#f87171", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11 }}>🗑 Limpiar todo</button>
        ...
        <div style={{ maxWidth: 1040, margin: "0 auto", padding: "20px 16px" }}>
          ...
          {tabVista === "subidas" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {subidas.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "#475569", fontSize: 13 }}>Sin facturas subidas aún</div>}
              {subidas.map((f, i) => (
                <div key={f.id+"_"+i} style={{ background: "#0f1a0f", border: "1px solid #166534", borderRadius: 10, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  <div><span style={{ fontWeight: 700, color: "#4ade80", fontSize: 13 }}>{f.razonSocial || f.nitProveedor}</span><div style={{ fontSize: 11, color: "#475569", marginTop: 3 }}>Fecha: {f.fecha} · Total: <span style={{ color: "#4ade80", fontWeight: 600 }}>${(f.total||0).toLocaleString("es-CO")}</span></div></div>
            ...
            <div>
              <div className="dz" onDragOver={e => { e.preventDefault(); e.currentTarget.classList.add("over"); }} onDragLeave={e => e.currentTarget.classList.remove("over")} onDrop={e => { e.preventDefault(); e.currentTarget.classList.remove("over"); recibirArchivos(e.dataTransfer.files); }} onClick={() => document.getElementById("fi").click()}>
                ...
                {procesando
                  ...
                  : <div><div style={{ fontSize: 34, marginBottom: 8 }}>📂</div><div style={{ fontFamily: "sans-serif", fontSize: 14, fontWeight: 600, color: "#cbd5e1" }}>Arrastra facturas XML o PDF aquí</div>{empresaActual && <div style={{ fontSize: 11, color: "#475569", marginTop: 4 }}>Empresa: <span style={{ color: "#60a5fa" }}>{empresaActual.nombre}</span> · {pucCuentas.length} cuentas</div>}</div>}
              </div>
              <div style={{ marginTop: 10 }}>
                <button onClick={() => setTestAbierto(a => !a)} style={{ background: "transparent", border: "1px dashed #2d3f6e", color: "#60a5fa", borderRadius: 6, padding: "4px 12px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>🧪 {testAbierto ? "Ocultar ▲" : "Panel de pruebas ▼"}</button>
                {testAbierto && (
                  <div style={{ background: "#131c2e", border: "1px dashed #2d3f6e", borderTop: "none", borderRadius: "0 0 9px 9px", padding: "11px 13px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 9 }}>
                      <span style={{ fontSize: 10, color: "#475569" }}>4 facturas de prueba</span>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                      {FACTURAS_TEST.map(t => (
                        <div key={t.nombre} style={{ background: t.color, border: "1px solid #1e2a3a", borderRadius: 7, padding: "8px 10px", display: "flex", gap: 7, alignItems: "center" }}>
                          ...
                          <div style={{ flex: 1, fontSize: 12, color: "#e8eaf0", fontFamily: "sans-serif", fontWeight: 600 }}>{t.nombre}</div>
          ...
          {tabVista === "aprobadas" && (
            <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
              {aprobadasAcum.length === 0 && <div style={{ textAlign:"center",padding:40,color:"#475569",fontSize:13 }}>Sin facturas aprobadas</div>}
              {aprobadasAcum.length > 0 && (
                <div>
                  <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:8,paddingBottom:6,borderBottom:"1px solid #166534" }}>
                    ...
                    <span style={{ fontSize:11,fontWeight:700,color:"#4ade80",textTransform:"uppercase",letterSpacing:.5 }}>Aprobadas · listas para subir · {aprobadas.length}</span>
                  </div>
                  {aprobadasAcum.map((f, i) => <FacturaCard key={f.id} f={f} idx={i} onUpdate={upd} docNum={String(i+1)} onAprender={() => {}} />)}
                  <div style={{ background:"#0f1a2e",border:"1px solid #1e3a5f",borderRadius:10,padding:"16px 20px",marginTop:12 }}>
                    <div style={{ fontFamily:"sans-serif",fontWeight:700,fontSize:13,color:"#60a5fa",marginBottom:10 }}>{aprobadasAcum.length} facturas aprobadas</div>
                    <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14 }}>
                      {[["Total",fmt(aprobadasAcum.reduce((s,f)=>s+(f.total||0),0)),"#4ade80"],["Retenciones",fmt(aprobadasAcum.reduce((s,f)=>s+(f.retefuente||0),0)),"#f59e0b"],["IVA",fmt(aprobadasAcum.reduce((s,f)=>s+(f.totalIva||0),0)),"#818cf8"],["Neto",fmt(aprobadasAcum.reduce((s,f)=>s+((f.total||0)-(f.retefuente||0)),0)),"#34d399"]].map(([l,v,c])=>(
                        <div key={l} style={{background:"#0d1520",borderRadius:7,padding:"10px 13px"}}><div style={{fontSize:9,color:"#64748b",textTransform:"uppercase",letterSpacing:.8,marginBottom:4}}>{l}</div><div style={{fontSize:15,fontWeight:700,color:c}}>{v}</div></div>
                      ))}
                    </div>
